"""Transformation utilities without mujoco_py dependencies"""
...
def quat_multiply(quaternion1, quaternion0):
    ...
    Args:
        quaternion1 (np.array): (w,x,y,z) quaternion
        quaternion0 (np.array): (w,x,y,z) quaternion

    Returns:
        np.array: (w,x,y,z) multiplied quaternion
    """
...
def quat_conjugate(quaternion):
    ...
    Args:
        quaternion (np.array): (w,x,y,z) quaternion

    Returns:
        np.array: (w,x,y,z) quaternion conjugate
    """
    return np.array(
...
def quat_inverse(quaternion):
    ...
    Returns:
        np.array: (w,x,y,z) quaternion inverse
    """
...
def random_quat(rand=None):
    ...
    Returns:
        np.array: (w,x,y,z) random quaternion
    """
    if rand is None:
...
def mat2pose(hmat):
    ...
    Returns:
        2-tuple:
            ...
            - (np.array) (w,x,y,z) orientation array in quaternion form
    """
    pos = hmat[:3, 3]
...
def pos_quat_to_transformation(pos, quat):
    """
    Converts a pair of position and quaternion to transformation matrix
    ...
    homo_pose_mat[:3, :3] = quat2rotation(quat)
...
def quat2rotation(quaternion):
    """
    Converts given quaternion to rotation matrix.

    Args:
        quaternion (np.array): (w,x,y,z) vec4 float angles

    Returns:
        np.array: 3x3 rotation matrix
    """
    # awkward semantics for use with numba
    q = np.asarray(quaternion).copy().astype(np.float32)
...
def axisangle2quat(vec):
    ...
    Returns:
        np.array: (w,x,y,z) vec4 float angles
    """
    # Grab angle
...
def force_in_A_to_force_in_B(force_A, torque_A, pose_A_in_B):
    ...
    torque_B = -rot_A_in_B.T.dot(skew_symm.dot(force_A)) + rot_A_in_B.T.dot(
        torque_A
    )
...
def clip_rotation(quat, limit):
    ...
    Args:
        quat (np.array): (w,x,y,z) rotation being clipped
...
def quat_error(q1, q2):
    ...
    qerror = quat_multiply(q2, q1_inv)
    error = quat2axisangle(qerror)
    return qerror, error
...
def rotate_quat(q_A_in_B, vecB):
    """rotate frame A about vecB

    Args:
        q (np.array): the quaternion expressing the orientation
        vec (np.array)
    """
    qrel = axisangle2quat(vecB)
    return quat_multiply(qrel, q_A_in_B)
...
def multiply_pose(pos1, quat1, pos2, quat2):
    ...
    """
    pose1 = pos_quat_to_transformation(pos1, quat1)
    pose2 = pos_quat_to_transformation(pos2, quat2)
    pose_res = pose_in_A_to_pose_in_B(pose2, pose1)
    return mat2pose(pose_res)


def pose_inverse(pos, quat):
    pose = pos_quat_to_transformation(pos, quat)
    pinv = pose_inv(pose)
    return mat2pose(pinv)
